...
import {Well, FormControl,Col, Button,Row,Grid,FormGroup, HelpBlock } from 'react-bootstrap';
import { connect } from 'react-redux';
import { postNewComment } from '../../actions/currentCategory';
...
class Request extends Component{
    constructor(props){
        ...
        this.state = {
            newCommentBody: "",
            commentError:  {validationState: null, message: null},
        }
    ...
    postComment(e){
        ...
        let newComment = {
            "_id": {
                "$oid": date.toISOString()
              },
              "date": date.toISOString(),
              "body": this.state.newCommentBody,
              "name": this.props.auth.user.full_name,
              "managerResponse": null
        }

        if(this.state.newCommentBody){
            this.props.postNewComment(newComment,this.props.current_category._id_capabilitate,this.props.current_category.current_category._id,this.props.request_id,this.props.request_index)
            this.setState({newCommentBody: "",commentError:  {validationState: null, message: null}});
        }else{
            this.setState({commentError: {validationState:"error", message: "You can't post a empty comment!"}})
        }
    ...
    render(){
        return (
            <span>
           <Well  className="requestWell">

                <div className="requestTumblr">
                <Grid className="requestContainer" fluid={true}>
                <Row>
                ...
                <Row>
                    ...
                    <span>{this.props.request.authorName}</span>
                    </span>
                    </Col>
                </Row>
                <Row>
                    ...
                    <Col md={5} className="requestDate dateColumn">
                    ...
                    {this.props.request.managerResponse?
                        <RequestManagerResponse request={this.props.request} />
                        :
                        null}


                {this.props.request.comments.length!=0 ? 
                <div className="commentSection">
                <CustomScroll flex="1" keepAtBottom={true}>

                {this.props.request.comments.map(function(comment,index){
                    return(
                ...
                {this.props.posting_new_comment?
                    <LoadingAnimation />
                    :
                    null
                }   
                ...
                <div className="addCommentSection">
                            ...
                            <FormGroup controlId="formControlsTextarea" validationState={this.state.commentError.validationState}>
                            <Grid fluid={true}>
                            <Row>
                            <Col xs={6} md={10}>
                            <FormControl 
                            ...
                            />
                            {this.state.commentError.message && (<HelpBlock>{this.state.commentError.message}</HelpBlock>)}
                            </Col>
                            <Col xs={6} md={2}>
                            <Button type="submit" bsStyle="primary" bsSize="large" className="postCommentBtn">Post</Button>
                            </Col>
                            </Row>
                            </Grid>
                            </FormGroup>
...
const mapStateToProps = (state) => ({
    ...
    posting_new_comment: state.currentCategory.posting_new_comment,
    auth: state.auth,
})

export default connect(mapStateToProps, {postNewComment})(Request);